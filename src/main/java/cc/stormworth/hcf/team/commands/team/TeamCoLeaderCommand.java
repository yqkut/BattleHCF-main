...
public class TeamCoLeaderCommand {
    @Command(names = {"team coleader", "t coleader",
            "t co-leader", "team co-leader",
            "f co-leader", "fac co-leader",
            "faction co-leader", "f coleader",
            "fac coleader", "faction coleader"
    }, permission = "")
    public static void coleaderAdd(final Player sender, @Param(name = "player") final UUID promote) {
        final Team team = Main.getInstance().getTeamHandler().getTeam(sender.getUniqueId());
        if (team == null) {
    ...
    public static void coleaderRemove(final Player sender, @Param(name = "player") final UUID demote) {
        final Team team = Main.getInstance().getTeamHandler().getTeam(sender.getUniqueId());
        if (team == null) {
            sender.sendMessage(ChatColor.RED + "You must be in a team to execute this command.");
            return;
