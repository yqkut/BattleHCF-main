...
public class TeamCaptainCommand {
    ...
    public static void captainAdd(final Player sender, @Param(name = "player") final UUID promote) {
    ...
    public static void captainRemove(final Player sender, @Param(name = "player") final UUID demote) {
        final Team team = Main.getInstance().getTeamHandler().getTeam(sender.getUniqueId());
        if (team == null) {
            sender.sendMessage(ChatColor.RED + "You must be in a team to execute this command.");
            return;
