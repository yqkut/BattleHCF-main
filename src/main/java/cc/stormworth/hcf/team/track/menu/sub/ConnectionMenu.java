...
public class ConnectionMenu extends Menu {
    ...
    public Map<Integer, Button> getButtons(Player player) {
        ...
        buttons.put(getSlot(3, 1), Button.fromItem(new ItemBuilder(Material.DIAMOND_SWORD).name("&e&lMembers Connected").build(), other -> {
            player.closeInventory();
            other.sendMessage(CC.translate("&eLoading logs..."));

            TaskUtil.runAsync(Main.getInstance(), () -> new TrackerEntryMenu(TeamTrackerManager.loadEntry(team, TeamActionType.MEMBER_CONNECTED), new ConnectionMenu(team)).openMenu(other));
        }));


        buttons.put(getSlot(5, 1), Button.fromItem(new ItemBuilder(Material.SKULL_ITEM).name("&e&lMembers Disconnected").build(), other -> {
            player.closeInventory();
            other.sendMessage(CC.translate("&eLoading logs..."));

            TaskUtil.runAsync(Main.getInstance(), () -> new TrackerEntryMenu(TeamTrackerManager.loadEntry(team, TeamActionType.MEMBER_DISCONNECTED), new ConnectionMenu(team)).openMenu(other));
        }));
