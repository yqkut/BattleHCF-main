...
public class BuyConfirmation extends Menu {
  ...
  public Map<Integer, Button> getButtons(Player player) {
    ...
    buttons.put(getSlot(1, 1),
        Button.fromItem(
            new ItemBuilder(Material.WOOL).data((short) 5).setGlowing(true).name("&a&lYES!")
                .build(),
            onConfirm));
    ...
    buttons.put(getSlot(4, 1),
        Button.fromItem(new ItemBuilder(Material.REDSTONE_LAMP_ON).setGlowing(true)
            .name("&c&lWarning")
            .addToLore(
                "&7[&c⚠&7] &cAre you sure you want to buy",
                "&call items from this category?",
                "",
                "&eSelect an option!"
            )
            .build()));
    ...
    buttons.put(getSlot(7, 1),
        Button.fromItem(
            new ItemBuilder(Material.WOOL).setGlowing(true).data((short) 14).name("&c&lNOPE!")
                .build(),
            (other) -> menu.openMenu(player)));
