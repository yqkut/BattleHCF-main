...
public class TipsSpanishMenu extends PaginatedMenu {
  ...
  public int getPages(Player player) {
    return (Main.getInstance().getTipManager().getTips().size() - 1) / 27 + 1;
  }
  ...
  public Map<Integer, Button> getAllPagesButtons(Player player) {
    ...
    List<Tip> tips = Main.getInstance().getTipManager().getTips().stream()
        .filter(Tip::isEnable).collect(
            Collectors.toList());

    while (slot < 54 - 10 && tips.size() > index) {
      Tip tip = tips.get(index);

      buttons.put(slot++, Button.fromItem(tip.getSpanishItem(),
          (other) -> {
            player.closeInventory();
            player.sendMessage("");
            CC.translate(tip.getMessages_spanish()).forEach(player::sendMessage);
            player.sendMessage("");
          }));
